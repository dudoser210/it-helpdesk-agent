...
Локальная мультиагентная система для заявок сотрудников по Wi-Fi, принтерам, доступам и программам. LLM работает через уже установленный Ollama и `qwen3.5:latest`; данные не отправляются во внешние API.
...
|---|---:|---|
| `OLLAMA_MODEL` | `qwen3.5:latest` | тег уже скачанной модели |
...
Это учебный прототип: он не подключён к реальному IAM, CMDB или ticketing system и не выполняет команды на устройстве сотрудника. SQLite рассчитан на один экземпляр приложения. BM25 устойчив и не требует второй модели, но хуже понимает перефразирование, чем dense retrieval. План развития описан в отчёте.
